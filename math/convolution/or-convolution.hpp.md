...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 1 \"math/convolution/or-convolution.hpp\"\ntemplate <class T>\r\
    \nvoid fzt(vector<T> &a){\r\n  const int n = a.size();\r\n  for(int j = 0; (1\
    \ << j) < n; j++){\r\n    for(int i = 0; i < n; i++){\r\n      if(!(i >> j & 1))\
    \ a[i | 1 << j] += a[i];\r\n    }\r\n  }\r\n}\r\ntemplate <class T>\r\nvoid fmt(vector<T>\
    \ &a){\r\n  const int n = a.size();\r\n  for(int j = 0; (1 << j) < n; j++){\r\n\
    \    for(int i = 0; i < n; i++){\r\n      if(!(i >> j & 1)) a[i | 1 << j] -= a[i];\r\
  ...
  verifiedWith: []
documentation_of: math/convolution/or-convolution.hpp
layout: document
redirect_from:
- /library/math/convolution/or-convolution.hpp
- /library/math/convolution/or-convolution.hpp.html
